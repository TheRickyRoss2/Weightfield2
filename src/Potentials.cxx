...
Potentials::Potentials(int dimy, int dimx, double p, double w) // constructor 
{
  //cout << " CallC" << endl;
  double var = 0.1;
  SetPitchWidthXY(dimy, dimx, p, w, var, var);
  return;
}
//////////////////////////////////////////////////////////////////////
...
{
  for (int i = 0; i < XMAX; i++) {    // bottom electrode (backplane): set to Vbias for p-type strips, otherwise 0
  ...
  for (int kk = 1; kk < 2; kk++) {
    for (int j = 0; j < (int) count; j++)    // set strips
    {
      for (int k = 0; k < width; k++) {
        dpot[YMAX - kk][(int) (XMAX - (int) pitch * (int) count) / 2 + j * (int) pitch + (int) pitch / 2
            - (int) width / 2 + k] = (strips == NTYPE) ? vbias : 0.0;
        wpot[YMAX - kk][(int) (XMAX - (int) pitch * (int) count) / 2 + j * (int) pitch + (int) pitch / 2
            - (int) width / 2 + k] = 0.0;
        // cout << " doping " << k << " " << dpot[YMAX-1][(int)(XMAX-(int)pitch*(int) count)/2 + j*(int)pitch+(int)pitch/2-(int)width/2+k] << endl;
        if (j == ((int) count / 2)) {
          fix[YMAX - kk][(int) (XMAX - (int) pitch * (int) count) / 2 + j * (int) pitch + (int) pitch / 2
              - (int) width / 2 + k] = 2;
          if (ReadOutTopFlag)
            wpot[YMAX - kk][(int) (XMAX - (int) pitch * (int) count) / 2 + j * (int) pitch + (int) pitch / 2
                - (int) width / 2 + k] = 1.0;
...
{                // method for iterative calculation, see: http://en.wikipedia.org/wiki/Relaxation_%28iterative_method%29
  ...
  double h2 = biny * biny * 1e-12; // matrix square, needed to calculate  the iterative steps
  ...
  if (fabs(vdepl_irr) > vbias && !gui->GetDJOn()
      && ((bulk == PTYPE && strips == NTYPE) || (bulk == NTYPE && strips == PTYPE)))
    ymin = max(0., dist - dist * sqrt(vbias) / sqrt(fabs(vdepl_irr))) / biny;
  if (fabs(vdepl_irr) > vbias && !gui->GetDJOn()
      && ((bulk == PTYPE && strips == PTYPE) || (bulk == NTYPE && strips == NTYPE)))
    ymaxbd = dist * sqrt(vbias) / sqrt(fabs(vdepl_irr)) / biny;
  // cout << "ymin = " << ymin <<  " Vdepl_irr = " << vdepl_irr << endl;
    ...
    for (int x = 0; x < XMAX; x++) {
      ...
      for (int y = ymin; y < (ymaxbd - 1); y++) {
        ...
        if (!tempfix[y][x]) {
          ...
          //nicolo
          newpot.wpot[y][x] =
              (1. / biny * wpot[y + 1][x] + 1. / biny * wpot[y - 1][x] + 1. / binx * wpot[y][(x + 1 + XMAX) % (XMAX)]
                  + 1. / binx * wpot[y][(x - 1 + XMAX) % (XMAX)]) / (2. / (biny) + 2. / (binx));
          ...
          if (gui->CallGetDetType() == 0 || gui->CallGetDetType() == 2) {
            ...
            newpot.dpot[y][x] =
                (1. / biny * dpot[y + 1][x] + 1. / biny * dpot[y - 1][x] + 1. / binx * dpot[y][(x + 1 + XMAX) % (XMAX)]
                    +
      ...
      if (tempfix[YMAX - 1][x] == 0) {
        ...
        newpot.wpot[YMAX - 1][x] =
            (1. / binx * wpot[YMAX - 1][(x - 1 + XMAX) % (XMAX)] + 1. / binx * wpot[YMAX - 1][(x + 1 + XMAX) % (XMAX)]
        ...
        newpot.dpot[YMAX - 1][x] = 0.25
            * (dpot[YMAX - 1][(x - 1 + XMAX) % (XMAX)] + dpot[YMAX - 1][(x + 1 + XMAX) % (XMAX)] + dpot[YMAX - 2][x]
                + dpot[YMAX - 1][x]
                + h2 * (ECHARGE / (EPSILON * EPSILONR)
                    * dopyx[YMAX - 1][(x + 1 + XMAX) % (XMAX)]));   // drift potential
      }
...
{                // method to set the doping profile in dopyx
  ...
  poissonf = ((bulk == PTYPE) ? -1.0 : 1.0) * (2.0 * vdepl)
      / (dist * dist * 1e-12);     // right side of Poisson equation => Q/epsilon =  eN/epsilon, this is in 1/m^3
  poissondens = ((bulk == PTYPE) ? -1.0 : 1.0) * (2.0 * 30)
      / (dist * dist * 1e-12);     //  Vdepl = 30 V ==> Density for 10 kOhm*cm ==>P-doping= 4.5*10^11 n/m^3

  vdepl_irr = (poissonf) * dist * dist * 1e-12 / 2.;
  if (rflag == 0)
    cout << "Bulk doping = " << setprecision(2) << (poissonf) * (EPSILON * EPSILONR) / ECHARGE * 1e-6
         << "[N/cm^3]; V depletion = " << vdepl_irr << " [V]; " << endl;

  if (gui->GetAcceptorCreation())
    Acceptorpoissonf = -gfluence * gui->GetFluence() * 1e6 * ECHARGE / (EPSILON * EPSILONR); //always p-type, NEGATIVE
  ...
  if (gui->GetInitialDopRemoval() || gui->GetAcceptorCreation()) {
    ...
    if (rflag == 0)
      cout << "After irradiation: Bulk doping = " << setprecision(2)
           << (poissonf + Acceptorpoissonf) * (EPSILON * EPSILONR) / ECHARGE * 1e-6 << "[N/cm^3]; V depletion = "
           << vdepl_irr << " [V]; \n ";
  }
  ...
  for (int x = 0; x < XMAX; x++) {
    for (int y = 0; y < YMAX; y++) {
      ...
      if (gui->CallGetDJType() == 0 && gui->GetDJOn()) {
        if (y < DJZero * YMAX)
          dopyx[y][x] += 10. / 2.1 * DJCoef * (-poissondens * DJZero + poissondens * y / YMAX)
              * (EPSILON * EPSILONR) / ECHARGE;// h density
        else
          dopyx[y][x] +=
              ehratio * 10. / 2.1 * DJCoef * (-poissondens * DJZero + poissondens * y / YMAX) * (EPSILON * EPSILONR)
                  / ECHARGE; // e density
      } else if (gui->CallGetDJType() == 1 && gui->GetDJOn()) {
        if (y < (DJZero - 0.1) * YMAX)
          dopyx[y][x] += 1. / 2.1 * DJCoef * poissondens * cbrt(1. * y - DJZero * YMAX)
              * (EPSILON * EPSILONR) / ECHARGE;
        else if (y > (DJZero - 0.1) * YMAX && y < YMAX * (DJZero + 0.1)) dopyx[y][x] += 0.;
        else
          dopyx[y][x] +=
              1. / 2.1 * ehratio * DJCoef * poissondens * cbrt(1. * y - (DJZero + 0.1) * YMAX) * (EPSILON * EPSILONR)
                  / ECHARGE;
      }
...
//   tempfix=FixRestriktor();	// set fix matrix
  ...
  for (int i = 1; i < YMAX; i = i + 2)    // calculate mean value for grid points without 4 neighbours
  {
    for (int j = 1; j < XMAX; j = j + 2) {
      newpot.dpot[i][j] = 0.25 * (newpot.dpot[i - 1][j - 1] + newpot.dpot[i - 1][j + 1] + newpot.dpot[i + 1][j + 1]
          + newpot.dpot[i + 1][j - 1]);
      newpot.wpot[i][j] = 0.25 * (newpot.wpot[i - 1][j - 1] + newpot.wpot[i - 1][j + 1] + newpot.wpot[i + 1][j + 1]
          + newpot.wpot[i + 1][j - 1]);
    }
  }
  ...
  for (int j = 0; j < XMAX; j++) {
    newpot.dpot[0][j] =
        (strips == PTYPE) ? vbias : 0.0; // bottom electrode (backplane): set to Vbias for p-type strips, otherwise
  ...
  for (int i = 1; i < YMAX; i++)    // average over neighbours of grid points which are not set yet
  {
    for (int j = (i + 1) % 2; j < XMAX; j = j + 2)  //// depending on line (i), counter j starts at 0 or 1
    {
      ...
      if (tempfix[i][j] == 0) {
        ...
        if (i == 0) l = 1;
        if (XMAX != 1) {
          newpot.dpot[i][j] = 0.25
              * (newpot.dpot[i][(j - 2 + XMAX) % (XMAX - 1)] + newpot.dpot[i + 1 - f][j] + newpot.dpot[i - 1 + l][j]
                  + newpot.dpot[i][(j + XMAX) % (XMAX - 1)]);
          ...
          newpot.wpot[i][j] = 0.25
              * (newpot.wpot[i][(j - 2 + XMAX) % (XMAX - 1)] + newpot.wpot[i + 1 - f][j] + newpot.wpot[i - 1 + l][j]
                  + newpot.wpot[i][(j + XMAX) % (XMAX - 1)]);
...
void Potentials::Multigrid(void *wfgui, bool ReadOutTopFlag) {
  ...
  if (gui->GetGainShape() != 0) // add gain  layer
  {
    ...
    //      double VdepGainRegion= gui->GetGainRegionVdepletion();
    ...
    if (gui->GetGainShape() != 0)
      cout << "Gain layer region: " << gui->Getygainlayerlow() << " - " << gui->Getygainlayerhigh() << " micron \n";
    ...
    for (int y = 0; y < YMAX; y++) {
      ...
      if ((bulk == PTYPE && strips == PTYPE) || (bulk == NTYPE && strips == NTYPE)) {
        if (y * GetBinSizey() <= gui->Getygainlayerlow()) {
          ...
          DDop = 1;
          if (y * GetBinSizey() <= gui->Getygainlayerlow())
            if (y * GetBinSizey() > (ELECTRODE_DEPTH))
              ddout += DD - pow((gui->GetGainLength() - GetBinSizey()) / gui->GetGainLength(), gui->GetGainExp() + 2);
        }
      }
      ...
      if (y * GetBinSizey() >= gui->Getygainlayerlow()) {
        if (y * GetBinSizey() < (gui->Getygainlayerhigh())) {
          ...
          LocalDop = DopingRem * gui->GetDopinggainlayerValue(); // local doping in N/m^3
          dd += GetBinSizey();
          // cout << " gain layer thickness: " << dd << " doping = " << LocalDop << endl;
        }
        ...
        } else if ((bulk == PTYPE && strips == PTYPE) || (bulk == NTYPE && strips == NTYPE)) {
          DD = pow((gui->GetGainLength() - dd) / gui->GetGainLength(), gui->GetGainExp() + 2);
          DDop = pow((gui->GetGainLength() - dd) / gui->GetGainLength(), gui->GetGainExp()); // gain layer shape
        }

      }


      // cout << " gain layer DD: " << DD << " ddout = " << ddout << " y = " << y*GetBinSizey() << " Potential = " << VdepGainLayer*(DD+ddout) << endl;
      ...
      for (int x = 0; x < XMAX; x++) {
        if (bulk == PTYPE && strips == NTYPE) {
          if (y != YMAX - 1) {
            ...
            if (dpot[YMAX - 1][x] == vbias && dpot[YMAX - 1][(int) (x - GainRecessBins)] == vbias
                && dpot[YMAX - 1][(int) (x + GainRecessBins)] == vbias) {
  ...
  if (gui->GetInitialDopRemoval()) {
    ...
    //       cout << " After removal Doping/cm^3 = " << DopingRem*poissonf*(EPSILON*EPSILONR)/ECHARGE*1e-6 << endl; // <<== this is correct
    if (DopingRem < 0.00001) DopingRem = 0;
    cout << "Fraction of inital doping in the bulk still present = " << DopingRem << " Final bulk doping: "
         << DopingRem * poissonf * (EPSILON * EPSILONR) / ECHARGE * 1e-6 << "  N/cm^3 " << endl;
  }
