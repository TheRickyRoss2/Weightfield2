...
void CalculateFields(Potentials &pot, Field **wf, Field **df) {
  ...
  for (int i = 1; i < pot.GetXMAX(); i++) {
    for (int j = 1; j < pot.GetYMAX() - 1; j++) {
      ...
      wf[j][i].SetField(
          -(pot.Getwpot(j, i) - pot.Getwpot(j, (i - 1 + pot.GetXMAX()) % (pot.GetXMAX()))) / (pot.GetBinSizex() * 1e-6),
    ...
    for (int y = pot.GetYMAX() - step / 2; y < pot.GetYMAX(); y++)
      df[y][i].SetField(df[pot.GetYMAX() - step / 2 - 1][i].GetFieldx(),
                        df[pot.GetYMAX() - step / 2 - 1][i].GetFieldy());
    //
...
void CalculateAbsFields(Potentials &pot, Field **wf, Field **df) {
  for (int i = 0; i < pot.GetXMAX(); i++) {
    for (int j = 0; j < pot.GetYMAX(); j++) {
      df[j][i].Setabs(sqrt(
          (df[j][i].GetFieldx()) * (df[j][i].GetFieldx()) + (df[j][i].GetFieldy()) * (df[j][i].GetFieldy())));
      wf[j][i].Setabs(sqrt(
          (wf[j][i].GetFieldx()) * (wf[j][i].GetFieldx()) + (wf[j][i].GetFieldy()) * (wf[j][i].GetFieldy())));
...
void RotateField(Potentials &pot, Field **rotatedf, Field **df, double angle) {
  ...
  for (int i = 0; i < pot.GetYMAX(); i++) {
    for (int j = 0; j < pot.GetXMAX(); j++) {
      rotatedf[i][j].SetField(cos(angle) * (cos(angle) * df[i][j].GetFieldx() - sin(angle) * df[i][j].GetFieldy()),
                              cos(angle) * (sin(angle) * df[i][j].GetFieldx() + cos(angle) * df[i][j].GetFieldy()));
    }
  }
  for (int i = 0; i < pot.GetYMAX(); i++) {
    for (int j = 0; j < pot.GetXMAX(); j++) {
