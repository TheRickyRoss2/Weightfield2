...
{     // create electrons and holes, evenly distributed	
  ...
  int PairsPer5Micron[500];
  int NPairsPer5Micron[500];
	  ...
	  for (int il =0; il<=Path; il+=DistHisto)
	      ...
	      else 
		{
		  if (ConstQFlag && ! UserUniformQFlag)  
		    ...
		    {
		      NPairsPer5Micron[ll] = AvPair*5.*exp(-1.*((Path-ll*DistHisto)/Laser1024Range));
		    }
		}
...
void SetLorentz(Potentials &pot, Carriers *carriers, double b, double Temp, int DType) {
  ...
  double mu_e = 1.15*Mu_e(Temp, DType); // Hall Mobility = 1.15 * mobility for electronn
  double mu_h = 0.72*Mu_h(Temp, DType); // Hall Mobility = 0.72 * mobility for Hall
		...
		if (carriers[j].Getinside()!=0 && carriers[j].Getinside()!=-1)	//if charges are inside the detector
		  {
		    ...
		    int PosBinx = (int) (carriers[j].Getx()/pot.GetBinSizex());
		    int PosBiny = (int) (carriers[j].Gety()/pot.GetBinSizey());
			...
			if(gui->Getdiffusionon()
		      ...
		      
			  {
			    ...
			    DiffDistance = pow(2*mu*kB*Temp/(1.6e-19)*TIMEUNIT,0.5)*1e6; //it's a lenght!			    
			    // Diffusion along x
		    ...
		    // cout << "cBinx - biny = "<< binx << " " << biny << " Gety " << carriers[j].Gety() << " binsize " << pot.GetBinSizey() << endl ;
			...
			if (PosBinyNew > PosBiny)			  
			    for (int kk = PosBiny; kk< PosBinyNew; kk++)
			      {
				LocalE = (dfield[kk][PosBinx].GetFieldy()+dfield[kk-1][PosBinx].GetFieldy())/2.;
		    ...
		    /*
		      ...
		      if (ynew +deltax > ELECTRODE_DEPTH+1 && ynew+deltax< pot.GetYMAX()*pot.GetBinSizey()-ELECTRODE_DEPTH-1 && ( ynew+deltax < ygainlow || ynew+deltax > ygainhigh)  )
		      {
		      // needs smoothing
